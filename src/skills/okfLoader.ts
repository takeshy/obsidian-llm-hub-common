...
import { getNodeFs, isAbsolutePath, normalizePathSeparators } from "./pathAccess.js";

/** One document of the help bundle a plugin generates and ships with itself. */
...
/** The help bundle this plugin ships. Each plugin documents itself, so the host provides it. */
export interface BuiltinOkfBundle {
  id: string;
  name: string;
  documents: readonly BuiltinOkfDocument[];
}

let builtinOkf: BuiltinOkfBundle = { id: "builtin:help", name: "Help", documents: [] };

/** Call once at plugin load, before anything reads the builtin bundle. */
export function configureBuiltinOkf(bundle: BuiltinOkfBundle): void {
  builtinOkf = bundle;
}
...
export interface OkfBundle {
  ...
  name: string;
  /** True for the generated help bundle shipped with the plugin. */
  builtin?: boolean;
}
...
/** Build the prompt fragment for the help bundle this plugin ships. */
export function buildBuiltinOkfSystemPrompt(): string {
  const index = builtinOkf.documents.find(doc => doc.path.toLowerCase() === "index.md");
  if (!index) return "";
  return `\n\n${OKF_SYSTEM_PROMPT_INTRO}\n${formatIndexSection(builtinOkf.id, builtinOkf.name, index)}`;
}
...
export async function readOkfDocument(app: App, root: string | null, bundleId: string, path: string): Promise<OkfDocument | null> {
  ...
  if (isBuiltinOkfBundleId(bundleId)) {
    const doc = builtinOkf.documents.find(candidate =>
