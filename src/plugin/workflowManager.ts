...
import { parseWorkflowFromMarkdown } from "../workflow/parser.js";
import type { WorkflowInput } from "../workflow/types.js";
...
import { promptForFile, promptForAnyFile, promptForNewFilePath } from "../ui/workflow/FilePromptModal.js";
...
import { canShowMcpApp, showMcpApp } from "../ui/mcpAppViewer.js";
...
export class WorkflowManager {
  ...
  constructor(plugin: PluginRuntime, selection: SelectionManager) {
    this.plugin = plugin;
  ...
  private async migrateLegacyWorkflowIds(): Promise<void> {
    const { hotkeys, triggers, hotkeysChanged, triggersChanged } =
      computeLegacyWorkflowIdMigration({
        enabledWorkflowHotkeys: workflowHost().getWorkflowHotkeys(),
        enabledWorkflowEventTriggers: workflowHost().getWorkflowEventTriggers(),
      });

    if (hotkeysChanged) workflowHost().setWorkflowHotkeys(hotkeys);
    if (triggersChanged) workflowHost().setWorkflowEventTriggers(triggers);
  }
  ...
  private doRegisterHotkeys(): void {
    for (const filePath of workflowHost().getWorkflowHotkeys()) {
  ...
  async executeFromHotkey(filePath: string): Promise<void> {
    ...
    const selection = this.selection.getLastSelection();
    const selectionLocation = this.selection.getSelectionLocation();
    ...
    try {
      ...
      const promptCallbacks = {
        ...
        // Only hosts that registered a viewer can show an MCP app.
        ...(canShowMcpApp() ? {
          showMcpApp: async (mcpApp: unknown) => {
            if (executionModal) showMcpApp(this.app, mcpApp);
          },
        } : {}),
      };
  ...
  async triggerStartupWorkflows(): Promise<void> {
    ...
    const triggers = workflowHost().getWorkflowEventTriggers();
  ...
  private async cleanupDeletedWorkflow(deletedPath: string): Promise<void> {
    const triggers = workflowHost().getWorkflowEventTriggers();
    const hotkeys = workflowHost().getWorkflowHotkeys();
    const newTriggers = triggers.filter((trigger) => trigger.workflowId !== deletedPath);
    const newHotkeys = hotkeys.filter((id) => id !== deletedPath);

    if (newTriggers.length !== triggers.length) workflowHost().setWorkflowEventTriggers(newTriggers);
    if (newHotkeys.length !== hotkeys.length) workflowHost().setWorkflowHotkeys(newHotkeys);
  }
