...
/**
 * Scan `text` for tool-call payloads naming one of `names`, and return them
 * along with the text those blocks were cut out of. Nothing recognisable means
 * no tool calls and `cleanedText` equal to the input.
 *
 * Names rather than definitions: the plugins declare their tools in different
 * shapes, and all this needs is which names count as a tool call.
 */
