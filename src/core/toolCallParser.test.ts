...
describe("extractInlineToolCalls", () => {
  it("returns no tool calls when text is empty", () => {
    const result = extractInlineToolCalls("", ["read_note"]);
...
This is telling the function to get only one message.`;
    const result = extractInlineToolCalls(text, ["mcp__ms365_get_shared_mailbox_message"]);
...
{"name": "read_note", "arguments": {"path": "notes/foo.md"}}
    ...
    const result = extractInlineToolCalls(text, ["read_note"]);
    expect(result.toolCalls).toHaveLength(1);
  ...
  it("detects [TOOL_CALLS] array (Mistral format)", () => {
    ...
    const result = extractInlineToolCalls(text, ["search_notes"]);
    expect(result.toolCalls).toHaveLength(1);
  ...
  it("detects <|python_tag|> wrapped JSON (Llama 3.1 native format)", () => {
    ...
    const result = extractInlineToolCalls(text, ["get_active_note"]);
    expect(result.toolCalls).toHaveLength(1);
...
\`\`\`
Let's see.`;
    const result = extractInlineToolCalls(text, ["list_notes"]);
    expect(result.toolCalls).toHaveLength(1);
