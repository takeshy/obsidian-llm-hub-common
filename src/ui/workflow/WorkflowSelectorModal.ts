...
import { loadFromCodeBlock, LoadResult, WorkflowBlockData } from "../../workflow/codeblockSync.js";
import { SidebarNode } from "../../workflow/types.js";
import { cls } from "../../core/classPrefix.js";
import { workflowHost } from "../../workflow/host.js";
const DEFAULT_WORKSPACE_FOLDER = "llm-hub";
const WORKFLOWS_FOLDER = "workflows";
import { getWorkflowNodeTypeLabels } from "../../workflow/index.js";
...
export class WorkflowSelectorModal extends Modal {
  ...
  onOpen(): void {
    ...
    contentEl.empty();
    contentEl.addClass(cls("workflow-selector-modal"));
    modalEl.addClass(cls("workflow-selector-modal-container"));

    // Load and sort files (workflows/ first), excluding workspace folder (chat history etc.)
    const wsFolder = workflowHost().getWorkspaceFolder() || DEFAULT_WORKSPACE_FOLDER;
    ...
    const lastPath = workflowHost().getLastSelectedWorkflow();
    ...
    // Search box
    const searchContainer = contentEl.createDiv({ cls: cls("workflow-selector-search") });
    this.searchInput = searchContainer.createEl("input", {
      type: "text",
      placeholder: t("workflowSelector.searchPlaceholder"),
      cls: cls("workflow-selector-search-input"),
      value: initialQuery,
    });
    ...
    const mainContent = contentEl.createDiv({ cls: cls("workflow-selector-main") });

    // Left pane: File list
    const leftPane = mainContent.createDiv({ cls: cls("workflow-selector-left-pane") });
    this.fileListEl = leftPane.createDiv({ cls: cls("workflow-selector-file-list") });
    this.renderFileList();

    // Right pane: Preview
    const rightPane = mainContent.createDiv({ cls: cls("workflow-selector-right-pane") });

    // Preview area
    this.previewEl = rightPane.createDiv({ cls: cls("workflow-selector-preview") });
    this.previewEl.setText(t("workflowSelector.selectFileToPreview"));

    // Buttons
    const buttonContainer = rightPane.createDiv({ cls: cls("workflow-selector-buttons") });
  ...
  private renderFileList(): void {
    ...
    if (this.filteredFiles.length === 0) {
      this.fileListEl.createDiv({
        cls: cls("workflow-selector-no-files"),
    ...
    for (const file of this.filteredFiles) {
      ...
      if (folder !== currentFolder) {
        ...
        const folderEl = this.fileListEl.createDiv({ cls: cls("workflow-selector-folder") });
        folderEl.createSpan({
          cls: cls("workflow-selector-folder-icon"),
          text: "\u25B6",
        });
        folderEl.createSpan({
          text: folder || "/",
      ...
      const fileItem = this.fileListEl.createDiv({
        cls: cls("workflow-selector-file-item"),
        attr: { tabindex: "0" },
      });
  ...
  private async selectFile(file: TFile): Promise<void> {
    ...
    workflowHost().setLastSelectedWorkflow(file.path);
  ...
  private renderWorkflowPreview(): void {
    ...
    if (result.data.name) {
      const nameEl = this.previewEl.createDiv({ cls: cls("workflow-selector-workflow-name") });
    ...
    const nodesContainer = this.previewEl.createDiv({ cls: cls("workflow-selector-nodes") });
  ...
  private renderAIWorkflowHistory(): void {
    ...
    const details = this.previewEl.createEl("details", {
      cls: cls("workflow-selector-history"),
      attr: { open: "" },
    });
    const summary = details.createEl("summary", { cls: cls("workflow-selector-history-summary") });
    summary.setText("AI workflow history");

    const historyList = details.createDiv({ cls: cls("workflow-selector-history-list") });
    ...
    for (const entry of recentEntries) {
      const historyItem = historyList.createDiv({ cls: cls("workflow-selector-history-item") });
      historyItem.setText(entry);
    }

    if (historyLines.length > 5) {
      historyList.createDiv({
        cls: cls("workflow-selector-history-more"),
