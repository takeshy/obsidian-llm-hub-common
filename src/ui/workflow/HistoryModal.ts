...
import { ExecutionRecord } from "../../workflow/types.js";
import { ExecutionHistoryManager, formatDuration, EncryptionConfig } from "../../workflow/history.js";
...
export class HistoryModal extends Modal {
  ...
  private renderPasswordPrompt(contentEl: HTMLElement): void {
    const container = contentEl.createDiv({ cls: cls("workflow-history-password-prompt") });

    container.createEl("p", {
      text: t("workflowModal.encryptedHistoryNeedsPassword"),
      cls: cls("workflow-history-password-message")
    });

    const inputContainer = container.createDiv({ cls: cls("workflow-history-password-input") });
  ...
  private renderHistoryUI(contentEl: HTMLElement): void {
    const container = contentEl.createDiv({ cls: cls("workflow-history-container") });

    // List panel
    const listPanel = container.createDiv({ cls: cls("workflow-history-list-panel") });

    // List header with title and controls
    this.listHeaderEl = listPanel.createDiv({ cls: cls("workflow-history-list-header") });
    this.renderListHeader();

    this.listEl = listPanel.createDiv({ cls: cls("workflow-history-list") });

    // Detail panel
    const detailPanel = container.createDiv({ cls: cls("workflow-history-detail-panel") });
    detailPanel.createEl("h3", { text: t("workflowModal.details") });
    this.detailEl = detailPanel.createDiv({ cls: cls("workflow-history-detail") });
  }
  ...
  private renderListHeader(): void {
    ...
    const titleRow = this.listHeaderEl.createDiv({ cls: cls("workflow-history-title-row") });
    titleRow.createEl("h3", { text: t("workflowModal.runs") });

    // Only show controls if there are records
    if (this.records.length > 0) {
      const controls = this.listHeaderEl.createDiv({ cls: cls("workflow-history-controls") });

      // Select all checkbox
      const selectAllLabel = controls.createEl("label", { cls: cls("workflow-history-select-all") });
      ...
      if (this.checkedRecordIds.size > 0) {
        const deleteBtn = controls.createEl("button", {
          ...
          cls: cls("workflow-history-delete-selected-btn"),
        });
  ...
  private renderList(): void {
    ...
    if (this.records.length === 0) {
      this.listEl.createDiv({
        cls: cls("workflow-history-empty"),
    ...
    for (const record of this.records) {
      const item = this.listEl.createDiv({ cls: cls("workflow-history-item") });
      if (this.selectedRecord?.id === record.id) {
        item.addClass("is-selected");
      }

      // Checkbox for selection
      const checkbox = item.createEl("input", { type: "checkbox", cls: cls("workflow-history-checkbox") });
      ...
      // Content wrapper
      const content = item.createDiv({ cls: cls("workflow-history-item-content") });

      const statusDot = content.createSpan({ cls: cls("workflow-history-status") });
      statusDot.addClass(`workflow-status-${record.status}`);

      const textContent = content.createDiv({ cls: cls("workflow-history-item-text") });

      const timeEl = textContent.createDiv({ cls: cls("workflow-history-time") });
      timeEl.textContent = new Date(record.startTime).toLocaleString();

      const metaRow = textContent.createDiv({ cls: cls("workflow-history-meta") });

      const durationEl = metaRow.createSpan({ cls: cls("workflow-history-duration") });
      durationEl.textContent = formatDuration(record.startTime, record.endTime);

      const statusEl = metaRow.createSpan({ cls: cls("workflow-history-status-text") });
  ...
  private renderDetail(): void {
    ...
    // Header info
    const header = this.detailEl.createDiv({ cls: cls("workflow-detail-header") });
    header.createDiv({ cls: cls("workflow-detail-row"), text: t("workflowModal.status", { status: record.status }) });
    header.createDiv({ cls: cls("workflow-detail-row"), text: t("workflowModal.started", { time: new Date(record.startTime).toLocaleString() }) });
    if (record.endTime) {
      header.createDiv({ cls: cls("workflow-detail-row"), text: t("workflowModal.duration", { duration: formatDuration(record.startTime, record.endTime) }) });
    }

    // Steps
    const stepsContainer = this.detailEl.createDiv({ cls: cls("workflow-detail-steps") });

    for (const step of record.steps) {
      const stepEl = stepsContainer.createDiv({ cls: cls("workflow-detail-step") });
      stepEl.addClass(`workflow-step-${step.status}`);

      const stepHeader = stepEl.createDiv({ cls: cls("workflow-step-header") });
      stepHeader.createSpan({ cls: cls("workflow-step-type"), text: `[${step.nodeType}] ${step.nodeId}` });

      const statusBadge = stepHeader.createSpan({ cls: cls("workflow-step-status") });
      statusBadge.addClass(`workflow-step-status-${step.status}`);
      statusBadge.textContent = step.status;

      if (step.timestamp) {
        const timeEl = stepHeader.createSpan({ cls: cls("workflow-step-time") });
        timeEl.textContent = new Date(step.timestamp).toLocaleTimeString();
      }

      if (step.input !== undefined) {
        const inputSection = stepEl.createDiv({ cls: cls("workflow-step-section") });
      ...
      if (step.output !== undefined) {
        const outputSection = stepEl.createDiv({ cls: cls("workflow-step-section") });
      ...
      if (step.error) {
        const errorEl = stepEl.createDiv({ cls: cls("workflow-step-error") });
      ...
      // MCP App button, for hosts that registered a viewer for it
      const mcpAppInfo = (step as { mcpAppInfo?: unknown }).mcpAppInfo;
      if (mcpAppInfo && canShowMcpApp()) {
        const mcpAppSection = stepEl.createDiv({ cls: cls("workflow-step-section") });
        ...
        mcpAppBtn.addEventListener("click", (e) => {
          e.stopPropagation();
          showMcpApp(this.app, mcpAppInfo);
        });
      }
      ...
      if (step.variablesSnapshot && this.onRetryFromError) {
        const retryStepBtn = stepEl.createEl("button", {
          ...
          cls: cls("workflow-step-retry-btn"),
        });
    ...
    // Actions
    const actions = this.detailEl.createDiv({ cls: cls("workflow-detail-actions") });
    ...
    const deleteBtn = actions.createEl("button", {
      cls: cls("workflow-detail-delete-btn"),
