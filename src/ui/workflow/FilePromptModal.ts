...
export class FilePromptModal extends Modal {
  ...
  onOpen(): void {
    ...
    contentEl.empty();
    contentEl.addClass(cls("workflow-file-prompt-modal"));
    this.component.load();
    ...
    // Preview container
    const previewContainer = contentEl.createDiv({ cls: cls("workflow-file-preview-container") });
    previewContainer.createEl("h4", { text: t("workflowModal.preview") });
    this.previewEl = previewContainer.createDiv({ cls: cls("workflow-file-preview") });
  ...
  private async showPreview(file: TFile): Promise<void> {
    ...
    if (ext === "md" || ext === "txt" || ext === "json" || ext === "csv") {
      ...
      const img = this.previewEl.createEl("img", {
        cls: cls("workflow-file-preview-image"),
      });
      ...
      this.previewEl.createDiv({
        ...
        cls: cls("workflow-file-preview-pdf"),
      });
      this.previewEl.createDiv({
...
class NewFilePathModal extends Modal {
  ...
  onOpen(): void {
    ...
    // Input container
    const inputContainer = contentEl.createDiv({ cls: cls("workflow-file-path-input") });
    ...
    if (this.extensions && this.extensions.length > 0) {
      contentEl.createDiv({
        ...
        cls: cls("workflow-file-path-hint"),
      });
    }
