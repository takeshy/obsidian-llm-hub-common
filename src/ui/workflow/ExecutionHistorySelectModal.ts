...
import type { ExecutionRecord, ExecutionStep, StepStatus } from "../../workflow/types.js";
...
export class ExecutionHistorySelectModal extends Modal {
  ...
  onOpen(): void {
    ...
    contentEl.empty();
    contentEl.addClass(cls("execution-history-select-modal"));
    ...
    const mainContent = contentEl.createDiv({ cls: cls("execution-history-select-content") });

    // Left column: Execution runs list
    const runsColumn = mainContent.createDiv({ cls: cls("execution-history-runs-column") });
    runsColumn.createEl("h3", { text: t("workflow.historySelect.recentExecutions") });
    this.runsListEl = runsColumn.createDiv({ cls: cls("execution-history-runs-list") });
    this.renderRunsList();

    // Right column: Steps for selected run
    const stepsColumn = mainContent.createDiv({ cls: cls("execution-history-steps-column") });
    stepsColumn.createEl("h3", { text: t("workflow.historySelect.steps") });
    this.stepsListEl = stepsColumn.createDiv({ cls: cls("execution-history-steps-list") });
    this.renderStepsList();

    // Buttons
    const buttonContainer = contentEl.createDiv({ cls: cls("execution-history-select-buttons") });
  ...
  private renderRunsList(): void {
    ...
    if (this.records.length === 0) {
      this.runsListEl.createDiv({
        cls: cls("execution-history-empty"),
    ...
    for (const record of this.records) {
      ...
      // Time and status
      const infoEl = runItem.createDiv({ cls: cls("execution-history-run-info") });
      infoEl.createDiv({
        cls: cls("execution-history-run-time"),
  ...
  private renderStepsList(): void {
    ...
    for (let idx = 0; idx < this.selectedRecord.steps.length; idx++) {
      ...
      const checkbox = stepItem.createEl("input", {
        type: "checkbox",
        cls: cls("execution-history-step-checkbox"),
      });
      ...
      // Step info
      const stepInfo = stepItem.createDiv({ cls: cls("execution-history-step-info") });

      // Header: type and id
      const headerEl = stepInfo.createDiv({ cls: cls("execution-history-step-header") });
      headerEl.createSpan({
        cls: cls("execution-history-step-type"),
        text: `[${nodeTypeLabels[step.nodeType] || step.nodeType}]`,
      });
      headerEl.createSpan({
        cls: cls("execution-history-step-id"),
        text: step.nodeId,
      });
      headerEl.createSpan({
      ...
      if (step.input && Object.keys(step.input).length > 0) {
        const inputEl = stepInfo.createDiv({ cls: cls("execution-history-step-preview") });
      ...
      if (step.status === "error" && step.error) {
        ...
        const outputEl = stepInfo.createDiv({ cls: cls("execution-history-step-preview") });
