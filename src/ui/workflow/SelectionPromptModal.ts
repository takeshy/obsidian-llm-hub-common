...
import { SelectionInfo, EditorPosition } from "../../workflow/types.js";
...
export class SelectionPromptModal extends Modal {
  ...
  onOpen(): void {
    ...
    contentEl.empty();
    contentEl.addClass(cls("workflow-selection-prompt-modal"));
    ...
    contentEl.createEl("p", {
      ...
      cls: cls("workflow-selection-instruction"),
    });

    // Textarea for content with selection capability
    const textareaContainer = contentEl.createDiv({ cls: cls("workflow-selection-textarea-container") });
    this.textareaEl = textareaContainer.createEl("textarea", {
      cls: cls("workflow-selection-textarea"),
      attr: {
        readonly: "true",
    ...
    // Selection info
    this.selectionInfoEl = contentEl.createDiv({ cls: cls("workflow-selection-info") });
