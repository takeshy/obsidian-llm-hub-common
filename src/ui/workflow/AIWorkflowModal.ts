...
import { findFileMentionOccurrences, findLiteralOccurrences, type MentionOccurrence } from "../../core/mentionResolver.js";
import { workflowHost, workflowTracing, type WorkflowChatChunk } from "../../workflow/host.js";
import type { StreamChunkUsage } from "../../core/usage.js";
import type { WorkflowAttachment } from "../../workflow/host.js";
import type { GenerationContext, SidebarNode, WorkflowNodeType, ExecutionStep } from "../../workflow/types.js";
import { findWorkflowBlocks, normalizeYamlText } from "../../workflow/parser.js";
import { ExecutionHistoryManager } from "../../workflow/history.js";
import { renderDiffView, createDiffViewToggle, formatLineComments, type DiffRendererState } from "./DiffRenderer.js";
import { WorkflowGenerationModal } from "./WorkflowGenerationModal.js";
import { showWorkflowPreview } from "./WorkflowPreviewModal.js";
import { showExecutionHistorySelect } from "./ExecutionHistorySelectModal.js";
import { getDiffViewModePreference, setDiffViewModePreference } from "../preferences.js";
...
class WorkflowConfirmModal extends Modal {
  ...
  onOpen(): void {
    ...
    contentEl.empty();
    contentEl.addClass(cls("ai-workflow-confirm-modal"));
    ...
    const scrollable = contentEl.createDiv({ cls: cls("ai-workflow-confirm-scrollable") });
    ...
    if (this.userRequest?.trim()) {
      const requestContainer = scrollable.createDiv({ cls: cls("ai-workflow-confirm-user-request") });
      requestContainer.createDiv({
        cls: cls("ai-workflow-confirm-user-request-label"),
        text: t("workflow.generation.yourRequest"),
      });
      requestContainer.createDiv({
        cls: cls("ai-workflow-confirm-user-request-body"),
    ...
    if (showExplanation && this.explanation) {
      const explanationContainer = scrollable.createDiv({ cls: cls("ai-workflow-explanation") });
      const header = explanationContainer.createDiv({ cls: cls("workflow-generation-section-header") });
    ...
    if (this.isSkill && this.newInstructions !== undefined) {
      const instrLabel = scrollable.createDiv({ cls: cls("edit-confirm-preview-label") });
      ...
      if (instrUnchanged) {
        scrollable.createDiv({
          cls: cls("ai-workflow-confirm-no-changes"),
          text: t("aiWorkflow.noChanges"),
        });
      } else {
        const instrWrapper = scrollable.createDiv({ cls: cls("ai-workflow-confirm-diff-wrapper", "ai-workflow-confirm-diff") });
    ...
    const diffLabel = scrollable.createDiv({ cls: cls("edit-confirm-preview-label") });
    ...
    if (yamlUnchanged) {
      ...
      const diffWrapper = scrollable.createDiv({ cls: cls("ai-workflow-confirm-diff-wrapper", "ai-workflow-confirm-diff") });
    ...
    // Buttons
    const buttonContainer = contentEl.createDiv({ cls: cls("ai-workflow-buttons") });
...
export class AIWorkflowModal extends Modal {
  ...
  private pendingAttachments: WorkflowAttachment[] = [];
  ...
  onOpen(): void {
    const { contentEl, modalEl } = this;
    contentEl.empty();
    contentEl.addClass(cls("ai-workflow-modal"));
    ...
    if (this.mode === "create") {
      ...
      const nameContainer = contentEl.createDiv({ cls: cls("ai-workflow-input-row") });
      ...
      this.nameInputEl = nameContainer.createEl("input", {
        type: "text",
        cls: cls("ai-workflow-name-input"),
        attr: {
      ...
      const pathContainer = contentEl.createDiv({ cls: cls("ai-workflow-input-row") });
      pathContainer.createEl("label", { text: t("aiWorkflow.outputPath") });
      const defaultPath = this.forceSkill
        ? `${workflowHost().getSkillsFolder()}/{{name}}`
        : this.defaultOutputPath || `${"workflows"}/{{name}}`;
      this.outputPathEl = pathContainer.createEl("input", {
        type: "text",
        cls: cls("ai-workflow-path-input"),
        value: defaultPath,
    ...
    contentEl.createEl("label", {
      text: descLabel,
      cls: cls("ai-workflow-label"),
    });

    // Description textarea container (for autocomplete positioning)
    const textareaContainer = contentEl.createDiv({ cls: cls("ai-workflow-textarea-container") });

    // Mention autocomplete dropdown
    this.mentionAutocompleteEl = textareaContainer.createDiv({
      cls: `${cls("autocomplete", "ai-workflow-mention-autocomplete")} is-hidden`,
    });

    // Description textarea
    this.descriptionEl = textareaContainer.createEl("textarea", {
      cls: cls("ai-workflow-textarea"),
      attr: {
        placeholder:
          this.mode === "create"
            ? this.forceSkill
    ...
    const attachmentRow = contentEl.createDiv({ cls: cls("ai-workflow-attachment-row") });
    ...
    const attachBtn = attachmentRow.createEl("button", {
      cls: cls("ai-workflow-attach-btn"),
      attr: { type: "button" },
    });
    ...
    this.attachmentsContainerEl = attachmentRow.createDiv({ cls: cls("ai-workflow-attachments") });

    // Show current workflow for modify mode
    if (this.mode === "modify" && this.existingYaml) {
      const details = contentEl.createEl("details", {
        cls: cls("ai-workflow-existing"),
      });
      details.createEl("summary", { text: t("aiWorkflow.currentWorkflow") });
      details.createEl("pre", {
        text: this.existingYaml,
        cls: cls("ai-workflow-yaml-preview"),
      });
    }

    // Model selection row
    const modelContainer = contentEl.createDiv({ cls: cls("ai-workflow-model-row") });
    modelContainer.createEl("label", { text: t("aiWorkflow.model") });

    this.modelSelect = modelContainer.createEl("select", {
      cls: cls("ai-workflow-model-select"),
    });

    const availableModels = workflowHost().getModelOptions();
    // Use last used model for AI workflow, or fall back to the host's current model
    const lastAIWorkflowModel = workflowHost().getLastWorkflowModel();
    const defaultModel = lastAIWorkflowModel && availableModels.some(m => m.value === lastAIWorkflowModel)
      ? lastAIWorkflowModel
      : workflowHost().getCurrentModel();
    ...
    if (this.mode === "modify") {
      const confirmContainer = contentEl.createDiv({ cls: cls("ai-workflow-confirm-row") });
      ...
      const executionHistoryRow = contentEl.createDiv({ cls: cls("ai-workflow-execution-history-row") });

      const executionHistoryBtn = executionHistoryRow.createEl("button", {
        cls: cls("ai-workflow-execution-history-btn"),
      });
      ...
      this.executionHistoryInfoEl = executionHistoryRow.createDiv({
        cls: cls("ai-workflow-execution-history-info")
      });
    ...
    // Status area
    this.statusEl = contentEl.createDiv({ cls: cls("ai-workflow-status") });
    ...
    this.pasteSectionEl = contentEl.createDiv({ cls: `${cls("ai-workflow-paste-section")} is-hidden` });
    ...
    const pasteButtonContainer = this.pasteSectionEl.createDiv({ cls: cls("ai-workflow-buttons") });
  ...
  private async generate(): Promise<void> {
    ...
    const selectedModel = this.modelSelect?.value as string;
    if (!selectedModel) {
      new Notice(t("aiWorkflow.selectModel"));
      return;
    }

    // The host lists what it can actually run; anything else is not configured.
    if (!workflowHost().getModelOptions().some(option => option.value === selectedModel)) {
  ...
  private async runGenerationLoop(
    ...
    const traceId = workflowTracing().traceStart("workflow-generation", {
      ...
      if (shouldPlan) {
        ...
        while (true) {
          ...
          const planResult = await this.runPlanningPhase(
            selectedModel, planRequest, workflowName, isSkill,
          ...
          if (isCancelled()) {
            ...
            workflowTracing().traceEnd(traceId, { metadata: { status: "cancelled" } });
            this.resolvePromise(null);
            return;
          }
...
Fix the problem and output ONLY the complete, valid YAML workflow starting with "name:". Do not include any prose, explanation, or commentary — just the YAML.`;
        ...
        let repaired = "";
        for await (const chunk of this.streamForWorkflow(
          selectedModel, repairPrompt, systemPrompt,
          abortController, traceId,
        )) {
          if (isCancelled()) break;
          if (chunk.type === "thinking" && chunk.content) {
      ...
      while (true) {
        ...
        const reviewResult = await this.runReviewPhase(
          selectedModel, currentRequest, plan || "", result.yaml, isSkill,
        ...
        const refinement = await this.runRefinementPass(
          selectedModel, currentRequest, plan || "", result.yaml, result.explanation,
      ...
      workflowHost().setLastWorkflowModel(selectedModel);
      ...
      if (needsDiffConfirmation) {
        ...
        if (confirmResult.result === "ok") {
          workflowTracing().traceEnd(traceId, { output: result.yaml });
          workflowTracing().score(traceId, { name: "status", value: 1, comment: "approved" });
          this.resolvePromise(result);
        } else if (confirmResult.result === "no") {
          workflowTracing().traceEnd(traceId, { output: result.yaml, metadata: { status: "revision-requested" } });
          workflowTracing().score(traceId, { name: "status", value: 0.5, comment: "revision requested" });
          ...
          workflowTracing().traceEnd(traceId, { metadata: { status: "cancelled" } });
          workflowTracing().score(traceId, { name: "status", value: 0.5, comment: "cancelled by user" });
          this.resolvePromise(null);
        }
        return;
      }
      ...
      workflowTracing().traceEnd(traceId, { metadata: { status: "error", error: message } });
      workflowTracing().score(traceId, { name: "status", value: 0, comment: message });
  ...
  /**
   * Stream a generation phase (plan, generate, review, refine) through the host's model layer.
   */
...
${currentRequest}${existingContext}`;
    ...
    try {
      ...
      for await (const chunk of this.streamForWorkflow(
        selectedModel, planUserPrompt, planSystemPrompt, abortController, traceId,
      )) {
...
${generatedYaml}`;
    ...
    try {
      ...
      let usage: StreamChunkUsage | undefined;
      for await (const chunk of this.streamForWorkflow(
        selectedModel, reviewUserPrompt, reviewSystemPrompt, abortController, traceId,
      )) {
        if (isCancelled()) return {};
...
${outputInstruction}`;
    ...
    try {
      ...
      let usage: StreamChunkUsage | undefined;
      for await (const chunk of this.streamForWorkflow(
        selectedModel, refinementPrompt, systemPrompt, abortController, traceId,
      )) {
        if (isCancelled()) return {};
  ...
  private getWorkflowSpec(): string {
    return workflowHost().getWorkflowSpecification();
  }
...
Output only the complete modified YAML, starting with "name:".`;
   ...
   */
  private formatExecutionSteps(steps: import("../../workflow/types.js").ExecutionStep[]): string {
  ...
  private renderAttachments(): void {
    ...
    this.pendingAttachments.forEach((attachment, index) => {
      const attachmentEl = this.attachmentsContainerEl!.createSpan({
        cls: cls("ai-workflow-attachment"),
      });
      ...
      const removeBtn = attachmentEl.createSpan({
        cls: cls("ai-workflow-attachment-remove"),
        text: "×",
      });
  ...
  private renderMentionAutocomplete(): void {
    ...
    this.mentionItems.forEach((item, index) => {
      const itemEl = this.mentionAutocompleteEl!.createDiv({
        cls: `${cls("autocomplete-item")} ${index === this.mentionIndex ? "active" : ""}`,
      });
      itemEl.createSpan({
        cls: cls("autocomplete-name"),
        text: item.value,
      });
      itemEl.createSpan({
        cls: cls("autocomplete-desc"),
