...
/** Keep no-discovery mode useful without turning direct reads into path guessing. */
export function buildNoDiscoverySystemPrompt({
  ...
  if (hasRagContext) {
    ...
    sourceGuidance = "RAG was requested, but it returned no relevant vault context. Do not fill the gap with assumptions or guessed file reads. You may use get_active_note_info when the user explicitly asks about the current note, and read_note only for an exact path explicitly supplied by the user.";
  } else {
